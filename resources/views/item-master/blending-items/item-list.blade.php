            ...
            <div class="row">
                <div class="col-6">
                    <div class="card_title_color text-dark text-start ps-2" style="font-size: 22px">
                        <strong>Blending Item Master</strong>
                    </div>
                </div>
                        ...
                        <table class="table measure_date_list">
                            ...
                            <tbody>
                                @if ($get_all_item->isNotEmpty())
                                    @foreach ($get_all_item as $get_the_blending_items_data)
                                        <tr>
                                            <td>{{ $get_the_blending_items_data->item_name }}</td>
                                            <td>
                                                {{ rtrim(rtrim(number_format($get_the_blending_items_data->weight, 2, '.', ''), '0'), '.') }} oz
                                            </td>
                                            <td>{{ $get_the_blending_items_data->ph_min }}</td>
                                            <td>{{ $get_the_blending_items_data->ph_max }}</td>
                                            <td>{{ $get_the_blending_items_data->temperature }}</td>

                                            @if ($get_the_blending_items_data->status === 'Active')
                                                <td class="text-center">
                                                    <span class="text-center new_date_status badge_completed">
                                                        Active
                                                    </span>
                                                </td>
                                            @elseif ($get_the_blending_items_data->status === 'Inactive')
                                                <td class="text-center">
                                                    <span
                                            ...
                                            @endif
                                            <td>
                                                <a href="{{ route('blending_edit_items', Crypt::encrypt($get_the_blending_items_data->id)) }}"
