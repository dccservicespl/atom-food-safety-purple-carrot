            ...
            <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            <div class="card_completed_quantity card-box">
                                <h4 class="fw-bold fs-6">In Process Quantity</h4>
                                <h2 class="fs-2 fw-bold">{{
                                    number_format($complete_quantity =
                                    $portioning_order_data->where('status','In Process')->sum('quantity'))
                                    }}</h2>
                            </div>
                        </div>
                        <div class="col-4">
                ...
                <div class="col-lg-4">
                    <div class="text-end">
                        ...
                        $complete = $portioning_order_data->where('status', 'In Process')->sum('quantity');
        ...
        <div class="container">
            <div class="" style="">
                <div class="accordion" id="accordionExample">
                    @forelse ($portioning_order_data as $orderDetailId => $measurements)
                    @php
                    $firstRow = $measurements[0];
                    $isFirst = $loop->first;
                    @endphp
                    <div class="accordion-item mb-3">
                        <h2 class="accordion-header" id="heading-{{ $orderDetailId }}">
                            <button
                                class="accordion-button {{ $isFirst ? 'collapsed bg-primary-subtle' : 'bg-primary-subtle' }}"
                                type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $orderDetailId }}"
                                aria-expanded="true" aria-controls="collapse-{{ $orderDetailId }}">
                                {{ $firstRow['order_detail']['component_details'] }}
                            </button>
                        </h2>
                        <div id="collapse-{{ $orderDetailId }}"
                            class="accordion-collapse collapse {{ $isFirst ? 'show' : '' }}"
                            aria-labelledby="heading-{{ $orderDetailId }}" data-bs-parent="#accordionExample">
                                    ...
                                    <div class="table-scroll-wrapper">
                                        <table class="{{ $is_ended ? 'component-table disabled' : 'component-table' }}">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><span class="letter-badge">{{ $log['order_detail']['letter']
                                                            }}</span></td>
                                                    <td>{{ $log['order_detail']['component_details'] }}</td>
                                                    <td>{{ $log['order_detail']['label'] }}</td>
                                                    <td>
                                                        <span class="weight-chip border border-secondary">
                                                            {{ $log['order_detail']['weight'] }}
                                                        </span>
                                                    </td>
                                                    <td>{{ !empty($log['start_time']) ? date('H:i A',
                                                        strtotime($log['start_time'])) : '—' }}</td>
                                                    <td>{{ !empty($log['end_time']) ? date('H:i A',
                                                        strtotime($log['end_time'])) : '—' }}</td>
