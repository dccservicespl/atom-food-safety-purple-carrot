            ...
            <div class="row">
                <div class="col-6">
                    <div class="card_title_color text-dark text-start ps-2" style="font-size: 22px">
                        <strong>Metal Detector Item Master</strong>
                    </div>
                </div>

                <div class="col-6 text-end pt-3">
                    <a href="{{ route('metal_detector_add_items') }}"
                        class="btn btn-outline-primary pt-2 pb-2 ps-5 pe-5 ms-4 me-4">
                        ...
                        <table class="table measure_date_list">
                            ...
                            <tbody>
                                @if ($get_all_item->isNotEmpty())
                                    @foreach ($get_all_item as $get_the_metal_detector_items_data)
                                        <tr>
                                            <td>{{ $get_the_metal_detector_items_data->item_name }}</td>
                                            <td>
                                                {{ rtrim(rtrim(number_format($get_the_metal_detector_items_data->weight, 2, '.', ''), '0'), '.') . ' ' . $get_the_metal_detector_items_data->item_unit }}
                                            </td>
                                            @if ($get_the_metal_detector_items_data->status === 'Active')
                                                <td class="text-center">
                                                    <span class="text-center new_date_status badge_completed">
                                                        {{ $get_the_metal_detector_items_data->status }}
                                                    </span>
                                                </td>
                                            @elseif ($get_the_metal_detector_items_data->status === 'Inactive')
                                                <td class="text-center">
                                                    <span
                                            ...
                                            @endif
                                            <td>
                                                <a href="{{ route('metal_detector_edit_items', Crypt::encrypt($get_the_metal_detector_items_data->id)) }}"
